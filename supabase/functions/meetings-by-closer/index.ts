...
serve(async (req) => {
  ...
  try {
    const { closer } = await req.json();

    if (!closer || (typeof closer === 'string' && closer.trim() === '')) {
      throw new Error('Parâmetro "closer" é obrigatório');
    }
    ...
    console.log('Executing meetings-by-closer query with:', { closer });
    const result = await client.queryObject(query, [closer]);
    const data = convertBigInt(result.rows);

    await client.end();

    return new Response(
      JSON.stringify({ success: true, data, rowCount: data.length }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  } catch (error) {
    console.error('Error in meetings-by-closer function:', error);

    if (client) {
      try { await client.end(); } catch (endError) { console.error('Error closing client:', endError); }
    }
    ...
    return new Response(
      JSON.stringify({ success: false, error: errorMessage, details: errorDetails }),
