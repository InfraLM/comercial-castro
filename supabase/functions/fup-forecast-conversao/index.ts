...
serve(async (req) => {
  ...
  try {
    ...
    if (!tipo || !VALID_TIPOS.includes(tipo)) {
      return new Response(
        JSON.stringify({ error: 'Invalid tipo. Use "sdr" or "closer"' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    
    console.log("Buscando conversão:", { data_inicio, data_fim, tipo, user_id: user.id });
